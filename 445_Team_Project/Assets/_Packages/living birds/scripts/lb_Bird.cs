...
public class lb_Bird : MonoBehaviour {
    ...
    //Custom Code
    //Added because birds would immediately fly off again. Seems like the sphere collider is
    //disabled on landing which would normally trigger a triggerExit call, leading to the bird flying away.
    IEnumerator TmpLock()
    {
        tmpPreventFlyAway = true;
        yield return new WaitForSeconds(1);
        tmpPreventFlyAway = false;
    }

	void OnTriggerExit(Collider col){
		//if bird has hopped out of the target area lets fly
        //Custom Code
		if (onGround && (col.tag == "lb_perchTarget") && !tmpPreventFlyAway)
        {
